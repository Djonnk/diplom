...
public class LemmaService {
    ...
    public Map<String, Integer> getLemmasFromText(String html) {
        ...
        for (String word : words) {
            if (word.isEmpty() || isServiceWord(word) || !isCyrillic(word)) {
                continue;
            }
            try {
    ...
    private boolean isServiceWord(String word) {
        try {
            ...
            return morphInfo.stream().anyMatch(info -> info.contains("СОЮЗ") || info.contains("ПРЕДЛ") || info.contains("МЕЖД") || info.contains("ЧАСТ"));
        } catch (Exception e) {
